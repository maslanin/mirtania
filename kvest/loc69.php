...
elseif($f['kvest_step'] == 8)
	...
	elseif($go == 1)
		{
		$nagr = rand(12,14) * $f['lvl'];
		$f['money'] += $nagr;
		$kvest['loc69']['date'] = $_SERVER['REQUEST_TIME'] + 86400;
		$f['kvest'] = serialize($kvest);
		$q = $db->query("update `users` set money='{$f['money']}',kvest_now=0,kvest_step=0,kvest='{$f['kvest']}' where id='{$f['id']}' limit 1;");
	...
	elseif($go == 2)
		{
		$nagr = rand($f['lvl'] * 1300, $f['lvl'] * 1500);
		$f['exp'] += $nagr;
		$kvest['loc69']['date'] = $_SERVER['REQUEST_TIME'] + 86400;
		$f['kvest'] = serialize($kvest);
		addexp($f['id'], $nagr);
