 ...
 *
 * ИСПРАВЛЕНО: убран бесполезный SELECT COUNT(*) в начале.
 */
...
if (date('d.m.Y', (int)$kvest['loc77st']['date']) == date('d.m.Y')) {
    msg2('Старик увлеченно рассматривает руны, ему явно не до вас...', 1);
}
...
if (empty($mod)) {
    ...
    if (0 < $runl && 0 < $runo && 0 < $runw && 0 < $runz && 0 < $runv) {
        ...
        $nagrada = mt_rand(50 * (int)$f['lvl'], 80 * (int)$f['lvl']);
        $f['money'] += $nagrada;
        $kvest = !empty($f['kvest']) ? @unserialize($f['kvest']) : [];
        if (!is_array($kvest)) $kvest = [];
        unset($kvest['loc77st']);
        $kvest['loc77st']['date'] = time();
        $f['kvest'] = serialize($kvest);
