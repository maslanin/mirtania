<?php
/**
 * Ксардас: квест на сердца големов.
...
if (empty($mod)) {
    msg('Приветствую тебя, ' . $f['login'] . '! Я Ксардас, один из самых сильных магов Миртании. Что привело тебя ко мне?');
    ...
    if (empty($go)) {
        ...
        if (date('d.m.Y', (int)$kvest['loc56ks']['date']) == date('d.m.Y')) {
        ...
        $nagr = mt_rand((int)$f['lvl'] * 30, (int)$f['lvl'] * 40);
        $f['money'] += $nagr;

        if (0 <= $f['lvl']) { $itm1 = 153; $itm2 = 191; }
        if (5 <= $f['lvl']) { $itm1 = 154; $itm2 = 192; }
        if (10 <= $f['lvl']) { $itm1 = 155; $itm2 = 193; }
        if (15 <= $f['lvl']) { $itm1 = 156; $itm2 = 194; }
        ...
        $db->query("UPDATE `users` SET `money` = " . (int)$f['money'] . ", `kvest` = '" . $db->real_escape_string($f['kvest']) . "' WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
        $str = 'Спасибо тебе, ' . $f['login'] . ', ты успешно ';
        if ($f['sex'] == 1) $str .= 'прошел'; else $str .= 'прошла';
        $str .= ' испытания. Как я и обещал, вот твоя награда<br/><br/>';
        $str .= '[получено ' . $nagr . ' монет, ' . $item1['name'] . ', ' . $item2['name'] . ']';
        msg2($str);
