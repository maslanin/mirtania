...
switch ($mod):
	...
	case 'lookbot':
		require_once('_sfdgh626xfy2j/class/DBCC.php');
		...
		while($a = $q->fetch_assoc())
		{
			...
			{
				echo ' (До '.date('d.m.Y H:i:s',$a['time']).')';
			}
			echo '</div>';
		}
		fin();
	...
	case 'vip':
		...
		$count = 0;
		if($q->num_rows == 0) msg('Список пуст', 1);
	...
	case 'blok':
		...
		$mysql = $mysql.'<br/>Заблокирован by '.$f['login'];
	...
	case 'chatban':
		...
		$mess = '<span style="color:'.$notice.';"><img src="pic/1429.png" alt=""/><b>'.$f['login'].': персонаж '.$lgn.' получает молчу на '.intval(($timeban - $t) / 60).' минут. ('.$mess.')</b></span>';
	...
	case 'givemoney':
		...
		$TestLGN = get_login($lgn);
		$lgn = $TestLGN['login'];
		$q = $db->query("update `users` set money=money+{$num} where login='{$lgn}' limit 1;");
	...
	case 'giveruda':
		...
		require_once('inc/i.php');
		$q = $db->query("insert into `letters` values(0,0,'{$t}','{$admin}','{$settings['bot']}','{$mess}',0,0);");
		$q = $db->query("update `users` set ruda=ruda+{$num} where login='{$lgn}' limit 1;");
	...
	case 'locedit':
		...
		if($q->num_rows == 0) msg('Нет такой локации', 1);
		$a = $q->fetch_assoc();
		if (empty($go))
			{
			echo '<div class="board" style="text-align:left">';
		...
		$q = $db->query("update `loc` set name='{$titl}', info='{$news}',peace='{$p}' where id='{$f['loc']}' limit 1;");
