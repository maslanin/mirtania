<?php
/**
 * Анкета персонажа, статы, VIP.
 * PHP 8.2-совместимая версия.
 *
 * ИСПРАВЛЕНО:
 *  - в mod=outklan была незакрытая кавычка в SQL (name='... limit 1)
...
$newinfa   = isset($_REQUEST['newinfa'])   ? $_REQUEST['newinfa'] : '';
$srok      = isset($_REQUEST['srok'])      ? (int)$_REQUEST['srok'] : 0;
$strelki   = isset($_REQUEST['strelki'])   ? (int)$_REQUEST['strelki'] : 0;
$stat_zdor  = isset($_REQUEST['stat_zdor'])  ? (int)$_REQUEST['stat_zdor'] : 0;
$stat_sila  = isset($_REQUEST['stat_sila'])  ? (int)$_REQUEST['stat_sila'] : 0;
$stat_inta  = isset($_REQUEST['stat_inta'])  ? (int)$_REQUEST['stat_inta'] : 0;
$stat_lovka = isset($_REQUEST['stat_lovka']) ? (int)$_REQUEST['stat_lovka'] : 0;
$stat_intel = isset($_REQUEST['stat_intel']) ? (int)$_REQUEST['stat_intel'] : 0;
...
switch ($mod) {
    case 'edit':
        ...
        if (30 < mb_strlen($newname, 'UTF-8') || mb_strlen($newname, 'UTF-8') < 3) {
        ...
        if (!empty($newpass) && !empty($newpass2)) {
            if (mb_strlen($newpass, 'UTF-8') < 9 || $newpass != $newpass2) {
            ...
            setcookie('hash', $f['pass'],       time() + 86400 * 365, '/');
        }
        if ($srok == 1) $f['grafika'] = 1;
        elseif ($srok == 2) $f['grafika'] = 2;
        elseif ($srok == 3) $f['grafika'] = 3;
        else $f['grafika'] = 0;
        if ($strelki == 1) $f['strelki'] = 1;
        elseif ($strelki == 2) $f['strelki'] = 2;
        else $f['strelki'] = 3;

        $name_esc = $db->real_escape_string($f['name']);
        $infa_esc = $db->real_escape_string($f['infa']);
        $pass_esc = $db->real_escape_string($f['pass']);
        $db->query("UPDATE `users` SET `name` = '{$name_esc}', `infa` = '{$infa_esc}', `pass` = '{$pass_esc}', `grafika` = '" . (int)$f['grafika'] . "', `strelki` = '" . (int)$f['strelki'] . "' WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
    ...
    case 'ref':
        ...
        $q = $db->query("SELECT `login`, `lvl`, `sex` FROM `users` WHERE `ref` = " . (int)$f['id'] . " AND `login` <> '" . $db->real_escape_string($f['login']) . "' ORDER BY `login`;");
        $count = 0;
        if ($q) {
            while ($s = $q->fetch_assoc()) {
                $color_login = ($s['sex'] == 1) ? $male : $female;
                $count++;
    ...
    case 'pvp':
        ...
        if ($f['pvpdate'] > $t) msg('Нельзя переключать данный статус чаще чем раз в неделю. Будет доступно ' . date('d.m.Y H:i', (int)$f['pvpdate']), 1);
        $new_pvp = ($f['pvp'] == 1) ? 0 : 1;
        $pvpdate = $t + 60 * 60 * 24 * 7;
        $db->query("UPDATE `users` SET `pvp` = {$new_pvp}, `pvpdate` = '{$pvpdate}' WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
    ...
    case 'otkaz':
        ...
        $db->query("UPDATE `users` SET `admin` = 0 WHERE `id` = " . (int)$f['id'] . " AND `admin` > 0 LIMIT 1;");
    ...
    case 'invite':
        ...
        if ($go == 1) {
            $inv_esc = $db->real_escape_string($f['klan_invite']);
            $q = $db->query("SELECT * FROM `klans` WHERE `name` = '{$inv_esc}' LIMIT 1;");
            if (!$q || $q->num_rows == 0) {
                msg2('Такого клана не существует.');
                $db->query("UPDATE `users` SET `klan_invite` = '' WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
                knopka('anketa.php', 'В анкету', 1);
                fin();
            }
            $k = $q->fetch_assoc();
            $db->query("UPDATE `users` SET `klan_invite` = '', `klan_time` = '{$t}', `klan` = '" . $db->real_escape_string($k['name']) . "', `klan_status` = 0, `nalog` = 0 WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
    ...
    case 'outklan':
        ...
        $db->query("UPDATE `users` SET `klan` = '', `klan_time` = '{$t}', `klan_status` = 0 WHERE `id` = " . (int)$f['id'] . " AND `klan` <> '' LIMIT 1;");
        // ИСПРАВЛЕНО: было name='{$f['klan']} limit 1 — незакрытая кавычка
        $db->query("UPDATE `klans` SET `kazna` = `kazna` + " . (int)$f['nalog'] . " WHERE `name` = '{$klan_esc}' LIMIT 1;");
        msg2('Ваш статус обновлен');
        knopka('anketa.php', 'В анкету', 1);
        fin();
    ...
    case 'razvod':
        ...
        $db->query("INSERT INTO `letters` VALUES (0, 0, '{$t}', '" . $db->real_escape_string($f['brak']) . "', '" . $db->real_escape_string($settings['bot']) . "', '" . $db->real_escape_string($mess) . "', 0, 0);");
        $db->query("UPDATE `users` SET `brak` = '' WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
        $db->query("UPDATE `users` SET `brak` = '' WHERE `login` = '" . $db->real_escape_string($f['brak']) . "' LIMIT 1;");
    ...
    case 'ressurect':
        ...
        if (empty($_SESSION['count']) || $_SESSION['count'] > 1) msg('Ошибка двойного нажатия', 1);
        $_SESSION['count']++;
        $db->query("UPDATE `users` SET `hpnow` = `hpmax`, `mananow` = `manamax`, `ruda` = `ruda` - 1 WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
    ...
    case 'kv':
        require_once __DIR__ . '/kvest/dnevnik.php';
        break;
    ...
    case 'stats':
        ...
        if ($stat_zdor < 1 || $stat_sila < 1 || $stat_inta < 1 || $stat_lovka < 1 || $stat_intel < 1) {
        ...
        $db->query("UPDATE `users` SET `zdor` = {$stat_zdor}, `sila` = {$stat_sila}, `inta` = {$stat_inta}, `lovka` = {$stat_lovka}, `intel` = {$stat_intel} WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
        $_SESSION['stats'] = 1;
    ...
    case 'vip':
        msg2('Руда: ' . $f['ruda']);
        if ($f['vip'] > $t) msg2('VIP статус до ' . date('d.m.Y H:i', (int)$f['vip']));
        if (empty($go)) {
            ...
            if (empty($ok)) {
                ...
                if (empty($_SESSION['count']) || $_SESSION['count'] > 1) msg('Ошибка двойного нажатия', 1);
                $_SESSION['count']++;
                $new_ruda = (int)$f['ruda'] - $cena;
                $new_vip = ($f['vip'] > $t) ? (int)$f['vip'] + 60 * 60 * 24 * 30 : $t + 60 * 60 * 24 * 30;
                $db->query("UPDATE `users` SET `vip` = {$new_vip}, `ruda` = {$new_ruda} WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
                ...
                if (empty($_SESSION['count']) || $_SESSION['count'] > 1) msg('Ошибка двойного нажатия', 1);
                $_SESSION['count']++;
                $new_ruda = (int)$f['ruda'] - $cena2;
                $new_vip = ($f['vip'] > $t) ? (int)$f['vip'] + 60 * 60 * 24 * 7 : $t + 60 * 60 * 24 * 7;
                $db->query("UPDATE `users` SET `vip` = {$new_vip}, `ruda` = {$new_ruda} WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
                msg2('VIP продлён на 7 суток за ' . $cena2 . ' руды');
            } else {
                msg('Нет такого варианта', 1);
            }
            knopka('anketa.php', 'В анкету', 1);
            fin();
        } elseif ($go == 3) {
            ...
            $db->query("UPDATE `users` SET `ruda` = `ruda` - {$cena} WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
            ...
            $q = $db->query("SELECT `login` FROM `users` WHERE `login` = '{$newname_esc}' LIMIT 1;");
            if ($q && $q->num_rows > 0) {
            ...
            if (empty($_SESSION['count']) || $_SESSION['count'] > 1) msg('Ошибка двойного нажатия', 1);
            $_SESSION['count']++;
            $old_esc = $db->real_escape_string($f['login']);
            $db->query("UPDATE `chat` SET `login` = '{$newname_esc}' WHERE `login` = '{$old_esc}';");
            $db->query("UPDATE `chat` SET `privat` = '{$newname_esc}' WHERE `privat` = '{$old_esc}';");
            $db->query("UPDATE `combat` SET `login` = '{$newname_esc}' WHERE `login` = '{$old_esc}';");
            $db->query("UPDATE `forum_comm` SET `login` = '{$newname_esc}' WHERE `login` = '{$old_esc}';");
            $db->query("UPDATE `forum_topic` SET `login` = '{$newname_esc}' WHERE `login` = '{$old_esc}';");
            $db->query("UPDATE `invent` SET `login` = '{$newname_esc}' WHERE `login` = '{$old_esc}';");
            $db->query("UPDATE `invent` SET `arenda_login` = '{$newname_esc}' WHERE `arenda_login` = '{$old_esc}';");
            $db->query("UPDATE `ipsoft` SET `login` = '{$newname_esc}' WHERE `login` = '{$old_esc}';");
            $db->query("UPDATE `letters` SET `login` = '{$newname_esc}' WHERE `login` = '{$old_esc}';");
            $db->query("UPDATE `letters` SET `login_from` = '{$newname_esc}' WHERE `login_from` = '{$old_esc}';");
            $db->query("UPDATE `log_peredach` SET `login` = '{$newname_esc}' WHERE `login` = '{$old_esc}';");
            $db->query("UPDATE `log_peredach` SET `login_per` = '{$newname_esc}' WHERE `login_per` = '{$old_esc}';");
            $db->query("UPDATE `magic` SET `login` = '{$newname_esc}' WHERE `login` = '{$old_esc}';");
            $db->query("UPDATE `users` SET `login` = '{$newname_esc}', `ruda` = `ruda` - 500 WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
    ...
    default:
        $f = calcparam($f);
        if (!empty($_REQUEST['dropqv']) && !empty($f['kvest_now'])) {
            $f['kvest_now'] = 0;
            $f['kvest_step'] = 0;
            $db->query("UPDATE `users` SET `kvest_now` = 0, `kvest_step` = 0 WHERE `id` = " . (int)$f['id'] . " LIMIT 1;");
        ...
        if ($f['hpnow'] < $f['hpmax'] && $f['ruda'] >= 1 && $f['status'] == 0) {
        ...
        echo '<br/>';
        if ($f['vip'] > $t) echo '<b>VIP-статус</b> до ' . date('d.m.Y H:i', (int)$f['vip']) . '<br/>';
        if (!empty($f['klan'])) echo '<a href="lib.php?mod=nalog">Налог: <b>' . $f['nalog'] . '</b></a><br/>';
        if (!empty($f['altar']) && $f['altar_time'] >= $t) echo 'Алтарь ' . $f['altar'] . ' уровня, еще ' . ceil(($f['altar_time'] - $t) / 60) . ' минут<br/>';
        ...
        if ($f['doping'] > 0 && $f['doping_time'] > $t) {
            ...
            echo 'Допинг: ' . ($doping_names[(int)$f['doping']] ?? '?');
        ...
        Уворот: ' . $f['uvorot'] . '<br/>
        Броня: ' . $f['bron'] . '<br/>';
        echo 'Урон: ' . (int)($f['uron'] * 1.4) . ' - ' . (int)($f['uron'] * 1.6) . '<br/>';
        $sopromag = (int)($f['bron'] * 0.1 + $f['intel']);
        ...
        if (!empty($f['klan'])) {
            $str = ($f['pvp'] == 1) ? '<span style="color:green">On</span>' : '<span style="color:red">Off</span>';
