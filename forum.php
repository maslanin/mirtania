...
if(empty($topic))
	...
	elseif($mod == 'close')
		...
		$q = $db->query("update `forum_topic` set flag_close=1 where id='{$topic}' limit 1;");
		msg2('Тема закрыта.');
		knopka('forum.php?razdel='.$razdel.'&topic='.$topic, 'В тему', 1);
		knopka('forum.php', 'Главная форума', 1);
		fin();
	...
	elseif($mod == 'open')
		...
		$q = $db->query("update `forum_topic` set flag_close=0 where id={$topic} limit 1;");
	...
	elseif($mod == 'delete')
		...
		if((($f['login'] == $MSG['login'] or $f['login'] == $FRM['login']) and $FRM['flag_close'] == 0) or $f['admin'] > 0)
			{
			$q = $db->query("DELETE FROM `forum_comm` WHERE id='{$cid}' AND id_topic='{$topic}' LIMIT 1;");
