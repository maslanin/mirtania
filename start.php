	...
	if ($f['soft'] != $soft or $f['ip'] != $ip)
		...
		if($q->num_rows > 0)
			...
			while($a = $q->fetch_assoc())
				{
				$logins .= '<a href="infa.php?mod=uzinfa&lgn='.$a['login'].'">'.$a['login'].'</a> - '.date('d.m.Y H:i', $a['date']).'<br/>';
				}
			$log = 'У игрока '.$f['login'].' совпал IP <b>'.$ip.'</b> со следующими персонажами:<br/>'.$logins;
			$q = $db->query("insert into `log_ipsoft` values(0,'{$f['login']}','{$t}','{$log}');");
			}
...
if(isset($_REQUEST['start']))
	...
	$r = isset($_REQUEST['r']) ? intval($_REQUEST['r']) : 1;			// это рефералы, кто пришел по hmr.su/start.php?start&r=12345
...
if(!empty($_SESSION['auth']))
	...
	if (empty($pass) or preg_match("/[^a-zA-Z0-9]/", $pass)) $err .= 'Неверно набран пароль. Допустимые символы <span style="color:darkgreen">a-Z 0-9</span><br/><br/>';
