...
if ($room == 3 and empty($f['klan'])) $room = 1;   // если не в клане, то перенос в общий чат
if ($room > 3 or $room < 1) $room = 1;
...
if ($mod == 'listchat')
	...
	while ($a = $q->fetch_assoc())
		...
		else echo '<a href="chat.php?lgn='.$a['login'].'"><b><span style="color:'.$color.'">'.$a['login'].'</span></b></a> <a href="chat.php?mod=view&lgn='.$a['login'].'">[i]</a> ['.date('H:i', $a['chatdate']).'] ';
		if ($a['chatroom'] == 1) echo '(общий)';
		else if ($a['chatroom'] == 2) echo '(торговый)';
		else echo '(клан)';
		echo '</div>';
		}
...
if ($room == 1 or $room == 2) $q = $db->query("select count(*) from `users` where chatdate>'{$timer}' and chatroom={$room};");
else if ($room == 3) $q = $db->query("select count(*) from `users` where chatdate>'{$timer}' and chatroom={$room} and klan='{$f['klan']}';");
$res = $q->fetch_assoc();
$res = $res['count(*)'];
