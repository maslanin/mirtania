...
elseif($mod == 'lot')
	...
	if($summa <= intval($item['price'] * 0.6)) msg2('Нельзя выставлять на продажу вещи ниже 60% от их цены.',1);
...
elseif($mod == 'search')
	...
	if(!empty($to))
		...
		if($slot == 1)
			...
			$a = $db->query("select count(`invent`.`id`) from `invent`,`item` where `invent`.`flag_rinok`=1 and `item`.`equip`='{$append}' and `invent`.`ido`=`item`.`id`;");
			$all_itm = $a->fetch_assoc(); // всего сообщений
			$all_itm = $all_itm['count(`invent`.`id`)'];
			if($start > intval($all_itm / $numb)) $start = intval($all_itm / $numb);
			if($start < 0) $start = 0;
			$limit = $start * $numb;
			$q = $db->query("select `invent`.`id` from `invent`,`item` where `invent`.`flag_rinok`=1 and `item`.`equip`='{$append}' and `invent`.`ido`=`item`.`id` order by `invent`.`time` desc limit {$limit},{$numb};");
			}
		}
	else
		{
		if($slot == 1)
			...
			$a = $db->query("select count(`invent`.`id`) from `invent`,`item` where `item`.`lvl`='{$lvl}' and `invent`.`flag_rinok`=1 and `invent`.`ido`=`item`.`id`;");
			$all_itm = $a->fetch_assoc(); // всего сообщений
			$all_itm = $all_itm['count(`invent`.`id`)'];
			if($start > intval($all_itm / $numb)) $start = intval($all_itm / $numb);
			if($start < 0) $start = 0;
			...
			$a = $db->query("select count(`invent`.`id`) from `invent`,`item` where `item`.`lvl`='{$lvl}' AND `item`.`equip`='{$append}' and `invent`.`flag_rinok`=1 and `invent`.`ido`=`item`.`id`;");
			$all_itm = $a->fetch_assoc(); // всего сообщений
			$all_itm = $all_itm['count(`invent`.`id`)'];
			if($start > intval($all_itm / $numb)) $start = intval($all_itm / $numb);
			if($start < 0) $start = 0;
			$limit = $start * $numb;
			$q = $db->query("select `invent`.`id` from `invent`,`item` where `item`.`lvl`='{$lvl}' AND `item`.`equip`='{$append}' and `invent`.`flag_rinok`=1 and `invent`.`ido`=`item`.`id` order by `invent`.`time` desc limit {$limit},{$numb};");
			}
		}
...
elseif($mod == 'kup')
	...
	$q = $db->query("update `users` set money=money+'{$item['rinok_price']}' where login='{$item['login']}' limit 1;");
